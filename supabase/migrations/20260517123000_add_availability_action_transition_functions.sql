...
begin
  ...
  set
    owner_status = case
      when owner_status = 'contacted' then 'contacted'
      else 'acknowledged'
    end,
...
create or replace function public.transition_checkout_created(
  p_action_id uuid,
...
begin
  ...
  if v_action.stripe_payment_intent_id is not null or v_action.payment_status = 'paid' then
    raise exception 'Availability action is already paid' using errcode = '23514';
  end if;

  if v_action.action_status in ('confirmed', 'cancelled', 'expired') then
    raise exception 'Availability action cannot start checkout from current state' using errcode = '23514';
  end if;

  if v_action.stripe_checkout_session_id is not null
     and v_action.stripe_checkout_session_id <> v_session_id then
    raise exception 'Availability action already has a different checkout session' using errcode = '23505';
  end if;

  if v_action.stripe_checkout_session_id = v_session_id
     and v_action.payment_status = 'checkout_created'
     and v_action.action_status = 'awaiting_payment' then
    return jsonb_build_object(
      'ok', true,
      'idempotent', true,
      'action', to_jsonb(v_action)
  ...
  set
    ...
    action_status = case
      when action_status in ('confirmed', 'cancelled', 'expired') then action_status
...
create or replace function public.transition_action_claimed(
  p_action_id uuid,
  p_action_type text
)
returns jsonb
...
begin
  ...
  v_requires_payment := v_action_type in ('pay', 'order');

  update public.availability_actions
  set
    action_status = coalesce(
      action_status,
      case when v_requires_payment then 'awaiting_payment' else 'claimed' end
    ),
    payment_status = coalesce(
      payment_status,
      case when v_requires_payment then 'pending' else 'not_required' end
    ),

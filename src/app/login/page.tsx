...
// Email + Password login (with Sign up + Reset) using browser-only Supabase client.
// Redirects to ?next=/path (defaults to /explore) after successful sign-in.
...
export default function LoginPage() {
  ...
  const next = (sp?.get('next') && decodeURIComponent(sp.get('next') as string)) || '/explore';
  ...
  const goNext = () => {
    if (typeof window !== 'undefined') {
      window.location.href = next || '/explore';
    }
  };
  ...
  const onSignup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error) return setErr(error.message);
      setMsg('Account created. Please check your email to confirm (if required), then sign in.');
      setMode('signin');
    } catch (e: any) {

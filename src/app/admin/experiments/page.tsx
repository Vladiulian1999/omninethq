...
export default function ExperimentsClient() {
  ...
  // for a small debug footer
  const [usedViews, setUsedViews] = useState<{ctr?: string; stats?: string; byTag?: string}>({});
  ...
  // Pick preferred -> fallback names for each window
  const pickViews = useCallback(() => {
    const win = (scope: 'all'|'30d'|'7d') => {
      const suffix = scope === 'all' ? '' : `_${scope}`;
      return {
        ctr: [`experiment_ctr${suffix}`, `success_experiment_ctr${suffix}`],
        stats: [`experiment_stats${suffix}`, `success_experiment_stats${suffix}`],
        byTag: [`experiment_ctr_by_tag${suffix}`], // legacy didn’t have this one
      };
    };
    return win(dateRange);
  }, [dateRange]);

  // Fetch first available view, remember which one worked
  const fetchFirstAvailable = useCallback(async <T,>(
    candidates: string[], kind: 'ctr'|'stats'|'byTag'
  ) => {
    for (const v of candidates) {
      const { data, error, status } = await supabase.from(v as any).select('*');
      if (!error && data) {
        setUsedViews(prev => ({...prev, [kind]: v}));
        return data as T[];
      }
      // 404 means relation doesn’t exist; try next
      if (status !== 404 && error) {
        // other errors (401/403) would indicate RLS or grants
        console.warn(`Query ${v} failed`, { status, error });
      }
    }
    setUsedViews(prev => ({...prev, [kind]: undefined}));
    return [] as T[];
  }, [supabase]);
  ...
  const loadData = useCallback(async () => {
    ...
    const [ctrData, statsData, byTagData] = await Promise.all([
      fetchFirstAvailable<CTRRow>(views.ctr, 'ctr'),
      fetchFirstAvailable<StatRow>(views.stats, 'stats'),
      fetchFirstAvailable<CTRByTagRow>(views.byTag, 'byTag'),
    ]);
    setCtr(ctrData || []);
    setStats(statsData || []);
    setByTag(byTagData || []);
  ...
  useEffect(() => { loadData(); }, [loadData, dateRange]);
    ...
    <div className="p-6 max-w-5xl mx-auto">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-2">
          <button onClick={loadData} className="text-sm rounded-xl border px-3 py-1 hover:bg-gray-50" disabled={loading}>
        ...
        {sortedCtr.map((row) => (
          <div key={row.variant} className="rounded-2xl border p-4 bg-white shadow-sm">
            ...
            <div className="mt-3 h-2 w-full bg-gray-100 rounded overflow-hidden">
              <div className="h-2 rounded" style={{ width: `${Math.max(0, Math.min(100, Number(row.ctr_percent || 0)))}%`, background: '#111' }} />
            </div>
          </div>
        ))}
        {sortedCtr.length === 0 && (
      ...
      <div className="mt-3 text-[11px] text-gray-400">
        Using views: ctr=<code>{usedViews.ctr || '—'}</code>,
        stats=<code>{usedViews.stats || '—'}</code>,
        byTag=<code>{usedViews.byTag || '—'}</code>
      </div>

      <div className="mt-4 text-xs text-gray-500">
        Tip: CTR = unique clicks / impressions. Windows are applied server-side.
      </div>
    </div>
  );
}

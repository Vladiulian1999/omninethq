...
export default function TemplatesPage() {
  ...
  useEffect(() => {
    const fetchTemplates = async () => {
      const { data, error } = await supabase.from('tag_templates').select('*').order('created_at', { ascending: false })
  ...
  const useTemplate = (template: TagTemplate) => {
    router.push(`/tag/new?title=${encodeURIComponent(template.title)}&description=${encodeURIComponent(template.description)}&category=${encodeURIComponent(template.category)}`)
  }
    ...
    <div className="max-w-3xl mx-auto p-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {templates.map((template) => (
          <div key={template.id} className="border rounded p-4 shadow bg-white">
            <h2 className="text-lg font-semibold mb-1">{template.title}</h2>
            <p className="text-sm text-gray-700 mb-2">{template.description}</p>
            <span className="inline-block text-xs bg-gray-200 text-gray-700 px-2 py-1 rounded mb-2">{template.category}</span>
            <button
